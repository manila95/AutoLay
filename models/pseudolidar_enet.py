...
class PseudoLidar_ENet(nn.Module):
    ...
    def __init__(self, opt, num_classes, encoder_relu=False, decoder_relu=True):
        super().__init__()

        self.opt = opt

        self.initial_block = InitialBlock(10, 16, padding=1, relu=encoder_relu)
    ...
    def forward(self, x):
        ...
        x = self.regular5_1(x)
        outputs["topview"] = self.transposed_conv(x)
        return outputs
    ...
    def loss(self, outputs, true_top_view):

        generated_top_view = outputs
        true_top_view = torch.squeeze(true_top_view.long())
        if self.opt.type == "lane":
            loss = nn.CrossEntropyLoss(weight=torch.Tensor([1., 3., 5., 7., 10., 5., 7., 10., 20.]).to(self.device))
        else:
            loss = nn.CrossEntropyLoss(weight=torch.Tensor([1., self.weight[self.opt.type]]).cuda())
